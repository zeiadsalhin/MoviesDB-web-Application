<template>
    <div class="main mt-12">
        <div class="main px-5 text-2xl mt- -mb-5 flex">
            <h1 class="my-auto">Top Rated TV Shows </h1>
            <router-link :to="{ name: 'top', params: { id: 'tv', name: 'Top Rated TV' } }"
        ...
        <div class="scrollh overflow-hidden md:m-3">
            <div class="scroll-container" ref="scrollContainer">
                ...
                <div class="movie-list" ref="movieList">
                    <div v-for="movie in visibleMovies" :key="movie.id" class="movie-item p-2"
                        ...
                        <router-link :to="{ name: 'Info', params: { id: movie.id } }">
                            ...
                            <h3 v-if="movie.name.length < 16" class="font-semibold md:text-lg p-4 mx-auto">
                                {{ movie.name }}
                            </h3>
                            <h3 v-else class="font-semibold md:text-lg p-4 mx-auto">{{ movie.name.substring(0, 15) +
                '..' }}
                            </h3>
                ...
                movie.vote_average.toFixed(1)
                            ...
                            <p class="opacity-70 text-sm">Released<br> {{ movie.first_air_date }}</p>
                        </router-link>
                    </div>
                </div>
                <button
...
export default {
    ...
    methods: {
        async fetchMovies() {
            ...
            try {
                const response = await fetch(`https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=${this.currentPage}`, options);
