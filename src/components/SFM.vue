<template>
    <div class="main mt-12">
        <div class="main px-5 text-2xl mt- -mb-5 flex">
            <h1 class="my-auto">Top Science Fiction Movies </h1>
            <router-link :to="{ name: 'genre', params: { id: 878, name: 'Top Science Fiction Movies' } }"
...
export default {
    ...
    methods: {
        async fetchMovies() {
            ...
            try {
                const response = await fetch(`https://api.themoviedb.org/3/discover/movie?&page=${this.currentPage}&with_genres=878`, options);
